...
import {  View, Animated, Easing, NativeModules, findNodeHandle, ViewPropTypes } from "react-native";
...
export default class ViewTransformer extends React.Component {
        ...
        NativeModules.UIManager.measure(handle, (x, y, width, height, pageX, pageY) => {
            if (typeof pageX === "number" && typeof pageY === "number") { // avoid undefined values on Android devices
                if (this.state.pageX !== pageX || this.state.pageY !== pageY) {
                    this._isMounted && this.setState({ pageX: pageX, pageY: pageY });
                }
            }
        });
    }
    ...
    onResponderGrant (evt, gestureState) {
        ...
        this._isMounted && this.setState({responderGranted: true});
        this.measureLayout();
    }
    ...
    performDoubleTapUp (pivotX, pivotY) {
        ...
        //  //   if (curScale > (1 + this.props.maxScale) / 2) { // former
        //    if (curScale > (1 + this.props.maxScale) / 2) { // former
    ...
    updateTransform (transform) {
        this._isMounted && this.setState(transform);
    }
